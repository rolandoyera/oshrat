    ...
    <section className="py-24 lg:py-32">
      <Container>
        ...
        <ScrollReveal direction="up" delay={200} threshold={0.1}>
          <div className="relative mx-auto mt-12 max-w-[900px] lg:mt-16">
            <DoubleBorder position="top" />
            {SERVICES.map((service) => (
              <Link
                key={service.index}
                href="/services"
                className="group relative grid grid-cols-[2.5rem_1fr_1.25rem] items-center gap-4 px-3 py-5 transition-colors hover:bg-cream-200 md:grid-cols-[3.5rem_1fr_auto_1.5rem] md:gap-8 md:px-4 md:py-7">
                <span className="text-xs uppercase tracking-[0.2em] text-accent font-bold">
                  {service.index}
                </span>
                <span>
                  <span className="block text-2xl font-normal leading-tight tracking-tight text-foreground lg:text-3xl">
                    {service.title}
                  </span>
                  <span className="mt-2 block text-xs uppercase tracking-[0.2em] text-foreground md:hidden">
                    <span className="mr-1.5 text-accent font-bold">•</span>
                    {service.category}
                  </span>
                </span>
                <span className="hidden whitespace-nowrap text-xs uppercase tracking-[0.2em] text-foreground md:block">
                  <span className="mr-1.5 text-accent font-bold">•</span>
                  {service.category}
                </span>
                <svg
